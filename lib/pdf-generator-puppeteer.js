...
<body>
  ...
  ${(mergedData?.areaWiseObservations || []).map((obs, index) => {
    ...
    const visualImg = sampleImages?.find(img => img.page === (index + 1)) || null;
    const thermalImg = thermalImages?.find(img => img.page === (index + 1)) || null;
    
    // Create absolute file paths for Puppeteer
    const visualPath = visualImg ? `file://${process.cwd()}/public${visualImg.path}` : null;
    const thermalPath = thermalImg ? `file://${process.cwd()}/public${thermalImg.path}` : null;
    ...
    // Log for debugging
    console.log(`Area ${index + 1}: Visual=${visualExists ? '✓' : '✗'} Thermal=${thermalExists ? '✓' : '✗'}`);
          ...
          <div class="image-comparison">
            ...
            <div class="image-container">
              ...
              <div class="image-wrapper">
                ${visualExists ? `<img src="${visualPath}" alt="Visual observation" />` : 
            ...
            <div class="image-container">
              ...
              <div class="image-wrapper">
                ${thermalExists ? `<img src="${thermalPath}" alt="Thermal reading" />` : 
                  `<div class="image-placeholder">
                    <div style="background: #F1F5F9; border: 2px dashed #CBD5E1; border-radius: 8px; padding: 30px;">
                      <p style="color: #64748B; font-weight: 600;">Image Not Available</p>
                    </div>
