...
function generateReportHTML(reportData) {
  ...
  const resolveImageSrc = (img) => {
    ...
    // Check if it's already a public URL (Supabase)
    if (img.path.startsWith('http')) {
      // For public URLs, we can return as is and Puppeteer will attempt to fetch it,
      // but Base64 is more reliable in serverless to avoid additional network delay inside the browser
      // However, for simplicity and performance on Vercel, public URLs are usually fine 
      // since the browser has internet access.
      return img.path;
    }
