...
// Helper to get Puppeteer for either local or serverless environments
async function getBrowser() {
  if (process.env.VERCEL) {
    // Vercel deployment uses puppeteer-core and @sparticuz/chromium
    const chromium = (await import('@sparticuz/chromium')).default;
    const puppeteer = (await import('puppeteer-core')).default;
    
    // Crucial for Vercel Node 20+ (AL2023) so Sparticuz unpacks correctly
    if (!process.env.AWS_LAMBDA_JS_RUNTIME) {
      process.env.AWS_LAMBDA_JS_RUNTIME = 'nodejs20.x';
    }
    
    // Explicitly grab the binary path
    const executablePath = await chromium.executablePath();
    
    // Crucial fix: Vercel node 20 lacks libnss3.so in the default path
    // Point the Linux loader explicitly to the Extracted Chromium directory where libraries reside
...
/**
 * Detect the Chromium/Chrome executable path based on the OS (Local Dev Only)
 */
function getChromePath() {
  const platform = process.platform;
  // ... rest of local detection ... (moved inside getBrowser logic really)
  if (platform === 'win32') {
    const candidates = [
  ...
  // Linux/Mac candidates... (keeping it brief)
  return undefined;
}
...
function generateReportHTML(reportData) {
  ...
  const resolveImageSrc = (img) => {
    ...
    // Check if it's already a public URL (Supabase) or a Base64 data URI
    if (img.path.startsWith('http') || img.path.startsWith('data:')) {
      // For public URLs or fully-encoded Base64 data, we can return as is
      return img.path;
    }

    // Build native OS path for existence check (Local Files)
    let nativePath;
    if (path.isAbsolute(img.path)) {
      nativePath = img.path;
    } else {
      nativePath = path.join(process.cwd(), 'public', img.path);
    }
