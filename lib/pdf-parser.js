        ...
        for (const img of images) {
          ...
          if (fs.existsSync(filePath)) {
            ...
            if (!uploadError) {
              ...
              // Fallback to Base64 data URI so the image still renders in the UI and PDF
              const base64Str = fileBuffer.toString('base64');
              const ext = path.extname(filePath).toLowerCase().replace('.', '') || 'png';
              uploadedImages.push({
                ...img,
                path: `data:image/${ext};base64,${base64Str}`
              });
            }
          }
        }
...
<svg width="400" height="300" xmlns="http://www.w3.org/2000/svg">
  ...
  <text x="50%" y="45%" text-anchor="middle" font-family="Arial" font-size="14" fill="#64748B">
    ${type === 'visual' ? 'Visual Inspection' : 'Thermal Imaging'} - Page ${i}
      ...
      const base64Str = Buffer.from(svgContent).toString('base64');
      const dataUri = `data:image/svg+xml;base64,${base64Str}`;
      
      images.push({ filename, path: dataUri, page: i, type, format: 'svg' });
    }
