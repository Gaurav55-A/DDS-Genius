    ...
    for (let pageIndex = 0; pageIndex < pages.length; pageIndex++) {
      ...
      for (const key of xObjectKeys) {
        try {
          ...
          if (xObject && xObject.dict && xObject.dict.get('Subtype')?.toString() === '/Image') {
            ...
            try {
              if (imageBytes && imageBytes.length > 0) {
                // Convert to buffer and save using sharp for better compatibility
                const imageBuffer = Buffer.from(imageBytes);
                await sharp(imageBuffer)
                  .jpeg({ quality: 85 })
                  .toFile(fullPath);
                
                console.log(`✓ Saved image: ${filename} (${imageBytes.length} bytes)`);
              ...
              console.log(`Could not save image ${filename}: ${saveError.message}`);
              // Still push metadata even if save fails - we'll show "Image Not Available" in PDF
              images.push({
                filename,
                path: imagePath,
                page: pageIndex + 1,
          ...
          console.log(`Could not extract image ${key} from page ${pageIndex + 1}: ${err.message}`);
        }
      }
    }
